    ...
    with app.app_context():
        ...
        try:
            ...
            cur.execute(
                ...
                VALUES (?, ?)
                ''',
                ('2024-12-31', 'Ayat Penyesuaian - Penyusutan Kendaraan')
            )
            ...
            cur.execute(
                ...
                VALUES (?, (SELECT id FROM accounts WHERE code = '504'), ?, ?, ?)
                ''',
                (adj_id, 200000, 0, 'Beban penyusutan kendaraan')
            )
            cur.execute(
                '''
                INSERT INTO adjusting_lines (adj_id, account_id, debit, credit, description)
                VALUES (?, (SELECT id FROM accounts WHERE code = '110'), ?, ?, ?)
                ''',
                (adj_id, 0, 200000, 'Akumulasi penyusutan kendaraan')
            )
            ...
            cur.execute(
                ...
                VALUES (?, ?)
                ''',
                ('2024-12-31', 'Ayat Penyesuaian - HPP & Persediaan Tiram')
            )
            adj_id = cur.lastrowid

            # HPP - Tiram Kecil / Persediaan - Tiram Kecil
            cur.execute(
                '''
                INSERT INTO adjusting_lines (adj_id, account_id, debit, credit, description)
                VALUES (?, (SELECT id FROM accounts WHERE code = '501'), ?, ?, ?)
                ''',
                (adj_id, 1180000, 0, 'Penyesuaian HPP tiram kecil')
            )
            cur.execute(
                '''
                INSERT INTO adjusting_lines (adj_id, account_id, debit, credit, description)
                VALUES (?, (SELECT id FROM accounts WHERE code = '105'), ?, ?, ?)
                ''',
                (adj_id, 0, 1180000, 'Penyesuaian persediaan tiram kecil')
            )

            # HPP - Tiram Besar / Persediaan - Tiram Besar
            cur.execute(
                '''
                INSERT INTO adjusting_lines (adj_id, account_id, debit, credit, description)
                VALUES (?, (SELECT id FROM accounts WHERE code = '502'), ?, ?, ?)
                ''',
                (adj_id, 1715000, 0, 'Penyesuaian HPP tiram besar')
            )
            cur.execute(
                '''
                INSERT INTO adjusting_lines (adj_id, account_id, debit, credit, description)
                VALUES (?, (SELECT id FROM accounts WHERE code = '106'), ?, ?, ?)
                ''',
                (adj_id, 0, 1715000, 'Penyesuaian persediaan tiram besar')
            )

            print("✅ Sample adjusting entries inserted successfully!")
...
def journal():
        ...
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center mb-4">
                ...
                <div>
                    ...
                    <button class="btn btn-outline-info ms-2" onclick="enhancedForceSave()">
                        <i class="fas fa-save me-2"></i>Simpan Sekarang
                    </button>
                </div>
            </div>
            ...
            <div class="alert alert-info d-flex align-items-center">
                <i class="fas fa-info-circle me-2"></i>
                <div>
                    <strong>Auto-save aktif</strong> - Data disimpan otomatis setiap 30 detik
                    <br><small class="text-muted">Terakhir disimpan: <span id="last-save-time">Beberapa detik yang lalu</span></small>
                </div>
            </div>
    ...
    for data in journal_data:
        ...
        body += f"""
                        ...
                        <tr class="journal-entry-header">
                            ...
                            <td colspan="3">
                                ...
                                <small class="text-muted ms-2">{entry['transaction_type']}</small>
                            </td>
        ...
        # Journal Lines
        for i, line in enumerate(lines):
            is_credit = line['credit'] > 0
            indent_class = "journal-credit" if is_credit else ""
            
            body += f"""
                        <tr class="journal-line {indent_class}">
                            <td></td>
                            <td class="{indent_class}">
                                {line['code']} - {line['name']}
                            </td>
                            <td></td>
                            <td class="text-end {'text-success fw-bold' if not is_credit else ''}">
                                {f"Rp {line['debit']:,.2f}" if line['debit'] > 0 else ""}
                            </td>
                            <td class="text-end {'text-danger fw-bold' if is_credit else ''}">
                                {f"Rp {line['credit']:,.2f}" if line['credit'] > 0 else ""}
                            </td>
                            <td></td>
                        </tr>
            """
        ...
        body += f"""
                        <tr class="journal-total">
                            <td colspan="3" class="text-end"><strong>Total Entri:</strong></td>
                        ...
                        <tr class="journal-spacer">
                            <td colspan="6" style="height: 10px; background: #f8f9fa;"></td>
                        </tr>
        """
    ...
    function updateSaveTime() {
        ...
        let timeText = 'Beberapa detik yang lalu';
        if (diffSec < 60) {
            timeText = `${diffSec} detik yang lalu`;
        } else {
            const diffMin = Math.floor(diffSec / 60);
            timeText = `${diffMin} menit yang lalu`;
        }
        
        document.getElementById('last-save-time').textContent = timeText;
    }
    
    function enhancedForceSave() {
        ...
        setTimeout(() => {
            ...
            btn.innerHTML = '<i class="fas fa-check me-2"></i>Disimpan!';
            ...
            alert.innerHTML = `
                ...
                <strong>Berhasil disimpan!</strong> Data telah disimpan ke database.
                <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
            `;
    ...
    // Update save time every 10 seconds
    ...
    setInterval(() => {
        ...
        alert.innerHTML = `
            ...
            <strong>Auto-save:</strong> Data telah disimpan otomatis.
            <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
        `;
        document.querySelector('.card-body').insertBefore(alert, document.querySelector('.alert-info'));
    }, 120000);
...
def journal_view(entry_id):
    ...
    body += f"""
            ...
            <div class="mt-4">
                <div class="alert {'alert-success' if abs(total_debit - total_credit) < 0.01 else 'alert-warning'}">
                    <i class="fas {'fa-check-circle' if abs(total_debit - total_credit) < 0.01 else 'fa-exclamation-triangle'} me-2"></i>
                    {'Entri ini seimbang dengan benar dan telah diposting ke buku besar.' if abs(total_debit - total_credit) < 0.01 else 'Entri ini tidak seimbang!'}
                </div>
            </div>
...
@login_required
def adjusting():
    cur = get_db().execute('SELECT * FROM adjusting_entries ORDER BY date DESC')
    entries = cur.fetchall()
    
    body = """
    <div class="page-header">
        <h1 class="page-title">Ayat Penyesuaian</h1>
        <p class="page-subtitle">Kelola ayat penyesuaian untuk penutupan periode</p>
    </div>
    ...
    if entries:
        ...
        body += """
            <div class="text-center py-5">
                ...
                <h5 class="text-muted">Tidak Ada Ayat Penyesuaian</h5>
                <p class="text-muted">Ayat penyesuaian digunakan untuk memperbarui saldo akun sebelum menyiapkan laporan keuangan.</p>
                <a href="/adjusting/new" class="btn btn-primary">Buat Ayat Penyesuaian</a>
            </div>
        """
